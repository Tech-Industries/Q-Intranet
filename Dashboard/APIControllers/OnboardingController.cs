...
namespace Dashboard.APIControllers
{
    public class OnboardingController : ApiController
    {
        ...
        [Route("api/v1/Onboarding/packages")]
        [HttpGet]
        public async Task<IHttpActionResult> GetPackages()
        {
            List<Package> packages = await onbdb.Packages.OrderBy(o => o.PackageName).ToListAsync();
        ...
        [HttpGet]
        public async Task<IHttpActionResult> GetOnboardingBurndown(string Program = null, int Task = 0, DateTime? startDate = null, DateTime? endDate = null)
        {
            if (Task == 0)
            {
                return BadRequest();
            ...
            var expec = new List<ExpectedDueCount>();
            var actual = new List<ActualCompCount>();

            if (startDate == null || endDate == null)
            {
                expec = await onbdb.ExpectedDueCounts.Where(x => x.PackageName == Program && x.Task == Task).OrderBy(o => o.DueBy).ToListAsync();
                actual = await onbdb.ActualCompCounts.Where(x => x.PackageName == Program && x.Task == Task).OrderBy(o => o.CompletedOn).ToListAsync();
            }
            else
            {
                expec = await onbdb.ExpectedDueCounts.Where(x => x.PackageName == Program && x.Task == Task && x.DueBy >= startDate && x.DueBy <= endDate).OrderBy(o => o.DueBy).ToListAsync();
                actual = await onbdb.ActualCompCounts.Where(x => x.PackageName == Program && x.Task == Task && x.DueBy >= startDate && x.DueBy <= endDate).OrderBy(o => o.CompletedOn).ToListAsync();
            }
            if (!expec.Any())
            {
                return NotFound();
            ...
            startDate = expec.First().DueBy;
            endDate = DateTime.Parse(expec.Last().DueBy.ToString()).AddDays(10).Date;
            ...
            var dates = await db.Calendars.Where(x => x.Date >= startDate && x.Date <= endDate).ToListAsync();
...
//            List<OnboardAuthorizedPart> authParts = new List<OnboardAuthorizedPart>();
//            List<OnboardPart> parts = new List<OnboardPart>();
...
//            if (RelPer.Any())
//            {
//                parts = await db.OnboardParts.OrderBy(o => o.DatePODue).Where(x => x.Archived != true).ToListAsync();
//                if (!parts.Any())
//                {
//                    return NotFound();
//                }
...
//            else
//            {
//                authParts = await db.OnboardAuthorizedParts.Where(x => x.DelegateUserID == id && x.Archived != true).OrderBy(o => o.DatePODue).ToListAsync();
//                if (!authParts.Any())
//                {
//                    return NotFound();
//                }
...
//        public async Task<IHttpActionResult> GetParts()
//        {
//            var parts = await db.OnboardParts.OrderBy(o => o.DatePODue).Where(x => x.Archived != true).ToListAsync();
//            if (!parts.Any())
//            {
//                return NotFound();
//            }
...
//            var timeSubmitted = DateTime.Now;
//            var oldPart = await db.OnboardParts.FindAsync(id);
...
//            try
//            {
//                db.OnboardParts.Remove(await db.OnboardParts.FindAsync(id));
//                await db.SaveChangesAsync();
//            }
//            catch (Exception e)
//            {
...
//        public async Task<IHttpActionResult> GetPartTasks()
//        {
//            var tasks = await db.OnboardTasks.ToListAsync();
//            if (!tasks.Any())
//            {
//                return NotFound();
//            }
...
//        public async Task<IHttpActionResult> GetPartTasksByID(int id)
//        {
//            var tasks = await db.OnboardTasks.Where(x => x.OnboardPartID == id && x.TaskSequence < 100).OrderBy(o => o.TaskSequence).ToListAsync();
//            if (!tasks.Any())
//            {
//                return NotFound();
//            }
...
//            var timeSubmitted = DateTime.Now;
//            var oldTask = await db.OnboardTasks.FindAsync(id);
...
//                var entry = db.Entry(oldTask);
//                if (oldTask.PercentComplete < 100 && task.PercentComplete == 100)
//                {
//                    oldTask.DateComplete = timeSubmitted;
//                    oldTask.DateComplete = task.DateComplete;
//                }
//                else if (task.PercentComplete < 100)
//                {
//                    oldTask.DateComplete = null;
//                    oldTask.DateComplete = null;
//                }

//                oldTask.DelegateUserID = task.DelegateUserID;
//                oldTask.Status = task.Status;
//                oldTask.PercentComplete = Convert.ToDecimal(task.PercentComplete);
//                oldTask.DateDue = task.DateDue;
//                oldTask.DateRecovery = task.DateRecovery;
